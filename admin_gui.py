...
# -*- coding: utf-8 -*-
"""
HIDDEN Yonsei Portfolio Admin Tool - Pro Version v2
전문 포트폴리오 관리 도구
  ...
  <footer class="site-footer">
    <div class="site-footer-inner">
      ...
      <div class="footer-bottom">
        <p class="footer-copyright">© 2026 HIDDEN Yonsei. All rights reserved.</p>
      </div>
    </div>
  </footer>
...
class BackupOptionsDialog(tk.Toplevel):
    ...
    def _create_ui(self):
        ...
        tk.Label(header, text="백업 유형 선택", font=ModernStyle.get_font(16, 'bold'),
                bg=ModernStyle.BG_WHITE).pack(anchor='w')
        tk.Label(header, text="백업 방식과 버전명을 선택하세요", font=ModernStyle.get_font(10),
                bg=ModernStyle.BG_WHITE, fg=ModernStyle.TEXT_SUBTLE).pack(anchor='w', pady=(5, 0))
        
        # 백업 유형 선택
        type_frame = tk.LabelFrame(self, text="백업 유형", font=ModernStyle.get_font(10, 'bold'),
                                   bg=ModernStyle.BG_WHITE, padx=15, pady=10)
        type_frame.pack(fill=tk.X, padx=25, pady=10)
        
        self.backup_type = tk.StringVar(value="changed")
        
        ttk.Radiobutton(type_frame, text="📝 변경된 파일만 백업", 
                       variable=self.backup_type, value="changed").pack(anchor='w', pady=3)
        tk.Label(type_frame, text="이전 백업과 비교하여 변경된 파일만 저장합니다.",
                font=ModernStyle.get_font(9), bg=ModernStyle.BG_WHITE, 
                fg=ModernStyle.TEXT_SUBTLE).pack(anchor='w', padx=20)
        
        ttk.Radiobutton(type_frame, text="📦 모든 파일 백업 (버전 스냅샷)", 
                       variable=self.backup_type, value="full").pack(anchor='w', pady=(10, 3))
        tk.Label(type_frame, text="현재 모든 HTML 파일을 완전히 백업합니다.",
                font=ModernStyle.get_font(9), bg=ModernStyle.BG_WHITE, 
                fg=ModernStyle.TEXT_SUBTLE).pack(anchor='w', padx=20)
        
        # 버전명 입력
        version_frame = tk.LabelFrame(self, text="버전명 (선택사항)", font=ModernStyle.get_font(10, 'bold'),
                                      bg=ModernStyle.BG_WHITE, padx=15, pady=10)
        version_frame.pack(fill=tk.X, padx=25, pady=10)
        
        tk.Label(version_frame, text="예: v1.0, 홈페이지수정, 프로젝트추가 등",
                font=ModernStyle.get_font(9), bg=ModernStyle.BG_WHITE, 
                fg=ModernStyle.TEXT_SUBTLE).pack(anchor='w')
        
        self.version_entry = ttk.Entry(version_frame, width=40, font=ModernStyle.get_font(10))
        self.version_entry.pack(fill=tk.X, pady=(8, 0))
        self.version_entry.insert(0, "")
        
        # 자동 버전 체크박스
        self.auto_version = tk.BooleanVar(value=True)
        ttk.Checkbutton(version_frame, text="버전 번호 자동 증가 (v1, v2, v3...)",
                       variable=self.auto_version, 
                       command=self._toggle_auto_version).pack(anchor='w', pady=(8, 0))
        
        # 버튼
        btn_frame = tk.Frame(self, bg=ModernStyle.BG_WHITE)
        btn_frame.pack(fill=tk.X, padx=25, pady=20)
        
        ttk.Button(btn_frame, text="취소", command=self.destroy).pack(side=tk.RIGHT, padx=(10, 0))
        ttk.Button(btn_frame, text="✓ 백업 생성", command=self._do_backup).pack(side=tk.RIGHT)
    
    def _toggle_auto_version(self):
        if self.auto_version.get():
    ...
    def _do_backup(self):
        ...
        version_name = self.version_entry.get().strip() if not self.auto_version.get() else None
        auto_version = self.auto_version.get()
...
class BackupManagerDialog(tk.Toplevel):
    ...
    def _restore_selected(self):
        ...
        path = self.backups.get(selected[0])
        if path:
            files = [f.name for f in path.glob("*.html")]
            if files:
...
class FooterEditorDialog(tk.Toplevel):
    ...
    def load_footer_data(self):
        ...
        try:
            ...
            self.data['logo'] = logo_match.group(1) if logo_match else 'HIDDEN'
...
class HomeManagerDialog(tk.Toplevel):
    ...
    def load_home_data(self):
        ...
        default_data = {
            ...
            "contact_email": "contact@hidden-yonsei.com",
            "hero_image": "",
            "hero_image_size": 100,
    ...
    def create_text_tab(self, parent):
        ...
        self._create_field(scroll_frame, "Contact Email", "email_entry",
                          "예: contact@hidden-yonsei.com", single_line=True)
...
class PortfolioAdminApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("HIDDEN Yonsei Portfolio Admin Pro")
    ...
    def create_ui(self):
        ...
        tk.Label(header, text="HIDDEN", font=ModernStyle.get_font(20, 'bold'),
                bg=ModernStyle.BG_WHITE).pack(side=tk.LEFT)
    ...
    def _do_backup_with_options(self, backup_type, version_name, auto_version):
        """옵션에 따른 백업 수행"""
        now = datetime.now()
        ...
        # 백업할 HTML 파일들
        html_files = [PROJECTS_HTML, DRAWINGS_HTML, GRAPHICS_HTML, ABOUT_HTML, INDEX_HTML]
        ...
        if backup_type == "full":
            ...
            for html in html_files:
                if html.exists():
                    shutil.copy(html, backup_folder / html.name)
                    backed_up.append(html.name)
            ...
            for html in html_files:
                if html.exists():
                    current_content = html.read_text(encoding='utf-8')
                    
                    prev_content = ""
                    if prev_backup:
                        prev_file = prev_backup / html.name
                        if prev_file.exists():
                    ...
                    if current_content != prev_content:
                        ...
                        change_summary = self._analyze_changes(html.name, prev_content, current_content)
                        if change_summary:
    ...
    def _get_latest_backup(self):
        ...
        for date_dir in BACKUP_DIR.iterdir():
            if date_dir.is_dir() and date_dir.name.isdigit() and len(date_dir.name) == 8:
                for time_dir in date_dir.iterdir():
                    if time_dir.is_dir() and time_dir.name.isdigit() and len(time_dir.name) == 6:
                        try:
                            dt = datetime.strptime(f"{date_dir.name}{time_dir.name}", "%Y%m%d%H%M%S")
...
또는 수동으로:
```
이 폴더의 HTML 파일들을 프로젝트 루트 폴더에 복사
```
            ...
            for filename in files_to_restore:
                ...
                if src.exists():
                    if filename == 'projects.html':
                        dst = PROJECTS_HTML
                    elif filename == 'drawings.html':
                        dst = DRAWINGS_HTML
                    elif filename == 'graphics.html':
                        dst = GRAPHICS_HTML
    ...
    def save_and_deploy(self):
        ...
        try:
            ...
            if not git.is_git_repo():
                ...
                # 첫 커밋 생성
                git.add_all()
                git.commit("Initial commit: HIDDEN Yonsei Portfolio")
